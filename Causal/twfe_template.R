# Boilerplate Two-Way Fixed Effects (TWFE) implementation of Diff-in-Diff that should be used when
...
report::report(pre_spend_model)

# TWFE
# All reported SEs and p-values are already cluster-robust at the city level.
twfe_nospend <- feols(
  sales ~ treat:post | city + date,
  data = sales_data, cluster = "city"
)

twfe_spend <- feols(
  sales ~ spend + treat:post | city + date,
  data = sales_data, cluster = "city"
)

summary(twfe_nospend)  

summary(twfe_spend)   

# Cluster-robust vcov table for TWFE without spend
vcov_cl_city_twfe_nospend = vcovCL(twfe_nospend, cluster = ~ city)

coeftest(twfe_nospend, vcov. = vcov_cl_city_twfe_nospend)

# Cluster-robust vcov table for TWFE with spend
vcov_cl_city_twfe_spend = vcovCL(twfe_spend, cluster = ~ city)

coeftest(twfe_spend, vcov. = vcov_cl_city_twfe_spend)
...
# Add a time index for city-specific linear trends
sales_data = sales_data %>%
  mutate(day_index = as.integer(date - min(date)))

# Simply add +spend to include spend
twfe_city_trend = feols(
  sales ~ treat * post | city[day_index] + date,
  data = sales_data,
  cluster = "city"
)
...
sales_data = sales_data %>%
  mutate(
    ...
    some_weight = 1  # placeholder weights; replace with something meaningful if desired
  )

twfe_season = feols(
  sales ~ treat * post | city + dow + month,
  data = sales_data,
  cluster = "city"
)
