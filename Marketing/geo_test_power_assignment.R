# Geo test powr table
# Assumes df has columns: dma, date, upgrades
...
#   - After exclusion, take the NEXT K DMAs by total upgrades, where K = n_test + n_ctrl.
#     (These K are the only DMAs eligible for random assignment; everyone else is BAU.)
# Power model:
#   - DMA-level DiD metric: (post_mean - pre_mean) in upgrades/day
#   - SD estimated as median SD across historical windows
#   - Two-sample t-test power with unequal n supported (pwr.t2n.test)
# Outputs:
#   - detectable_drop_upgrades_per_day (MDE) at target_power
#   - detectable_pct_drop = MDE / avg_daily_upgrades_per_DMA_in_eligible_pool
# Assumes using DiD: (Post−Pre)Test−(Post−Pre)Control
...
# Sample-size options (test/control counts)
...
# 1) Rank DMAs by total upgrades once
...
# include ranks 6–10 of the top10, exclude ranks 1–5 of the top10
top10_exclude_5 <- top10[1:5]

# 2) Build eligible pool: exclude some of top10, then take next K after exclusion
...
# 3) Estimate SD of DMA-level DiD metric for duration T (median across windows)
...
estimate_sd_did <- function(df, T_weeks) {
  ...
  did <- df %>%
    ...
    pivot_wider(names_from = period, values_from = mean_up) %>%
...
# 5) Run one power table for a given eligible pool & sample sizes
...
run_power_table <- function(df_eligible, durations_weeks, n_test, n_ctrl,
  ...
  # baseline for % expression
  avg_daily <- df_eligible %>%
    group_by(dma) %>%
    summarise(mean_daily = mean(upgrades, na.rm = TRUE), .groups = "drop") %>%
    summarise(mean(mean_daily)) %>%
    pull()
  ...
  map_dfr(durations_weeks, function(T) {
    ...
    tibble(
      weeks = T,
      sd_did = sd_did,
      detectable_drop_upgrades_per_day = mde,
      detectable_pct_drop = mde / avg_daily,
      avg_daily_upgrades = avg_daily,
...
results_all <- tidyr::crossing(
  ...
  select(
    ...
    weeks, sd_did, avg_daily_upgrades,
    detectable_drop_upgrades_per_day, detectable_pct_drop
  ) %>%
