...
# ---- Colab/Notebook-friendly setup ----
# Use a CRAN mirror explicitly (important in non-interactive environments like Colab)
...
# Helper: source optional repo scripts if present (works locally, in Colab, or anywhere)
safe_source <- function(path) {
  if (file.exists(path)) {
    source(path)
    return(invisible(TRUE))
  }
  message(sprintf("Skipping source('%s') (file not found).", path))
  invisible(FALSE)
}

# If you're running this in Colab, clone the repo (or upload these files) so these paths exist.
safe_source("R/Helpers/themes.R")
safe_source("R/Causal/causal_pre_check.R")
safe_source("R/Causal/causal_by_group_functions_dev.R")
```
