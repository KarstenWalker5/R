...
el_data <- bq_table_download(tb)%>%
  select(-uid, -reported_user_type, -country_code, -retained7d, -retained90d, 
         -cumulative_lifetime_sessions,-is_teacher_with_students, -session_count, -first_session_channel, -last_session_channel,
         -lifetime_sets_created,-never_created,-session_count_7d_avg, -set_pageviews_count_7d_avg, 
         -unique_sets_viewed, -set_pageviews_count)%>%
el_data<-el_data%>%  
...
el_data<-el_data%>%
  select(-reported_user_type, -country_code)
