...
cat("Users with NO cluster coverage at all:", nrow(DT %>%
                                                     group_by(user_id) %>%
...
saveRDS(
  fit_train,
  file = file.path(artifact_path, "el_fit28.rds")
)
...
saveRDS(
  model_meta,
  file = file.path(artifact_path, "model_meta28.rds")
)
...
write.csv(
  test_scored,
  file = file.path(artifact_path, "el28_run1_test_scored.csv"),
  row.names = FALSE
)

write.csv(
  train_data,
  file = file.path(artifact_path, "el28_run1_train.csv"),
  row.names = FALSE
)

write.csv(
  validation_data,
  file = file.path(artifact_path, "el28_run1_val.csv"),
  row.names = FALSE
)
